...
//    }
//}
// NpcReloaderGUI.Core.EldenRingConfig.cs
using System;
using System.Globalization;
using System.Linq;
...
namespace NpcReloaderGUI.Core
{
    public static class EldenRingConfig
    {
        // --- WorldChrMan Pointer Info (from DSAS INI) ---
        public static string WorldChrManAob { get; } = "48 8B 05 ?? ?? ?? ?? 48 85 C0 74 0F 48 39 88 ?? ?? ?? ?? 75 06 89 B1";
        public static int WorldChrManJumpStartOffset { get; } = 3;
        public static int WorldChrManJumpEndOffset { get; } = 7;

        // --- WorldChrMan Structure Offsets (from DSAS INI) ---
        // These are used to patch the shellcode.
        public static int AsmPatch_Offset1 { get; } = ParseHex("1E668");
        public static int AsmPatch_Offset2 { get; } = ParseHex("1E670");
        public static int AsmPatch_Offset3 { get; } = ParseHex("1E678");

        // --- Crash Fix Patch Info (from DSAS INI) ---
        ...
        public static int CrashFixOffsetDistanceFromEnd { get; } = 3;
        public static byte[] CrashFixWriteBytes { get; } = ParseHexString("4831D2"); // "XOR RDX, RDX"
        ...
        private static byte[] ParseHexString(string hexString)
        {
            if (string.IsNullOrWhiteSpace(hexString) || hexString.Length % 2 != 0) return new byte[0];
            return Enumerable.Range(0, hexString.Length / 2)
                             .Select(x => Convert.ToByte(hexString.Substring(x * 2, 2), 16))
                             .ToArray();
        }
